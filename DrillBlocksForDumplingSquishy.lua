-- [ NEXZAN HUB ] - AUTO FARM CHICKEN (V2.0.0)
-- Game: Steal a Chicken
...
function NexzanUI:Create(config)
    ...
    local mapName = "Drill Blocks for Dumpling Squishy"
pcall(function() mapName = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name end)
    ...
    local LoadingCenter = Create("Frame", { Size = UDim2.new(0, 320, 0, 90), Position = UDim2.new(0.5, -160, 0.5, -45), BackgroundColor3 = Color3.fromRGB(255, 255, 255), BorderSizePixel = 0, ClipsDescendants = true, Parent = LoadingFrame })
    ...
    local TitleArea = Create("Frame", {Size=UDim2.new(0, 250, 0, 20), Position=UDim2.new(0, 65, 0, 15), BackgroundTransparency=1, Parent=LoadingCenter})
    local SubtitleArea = Create("Frame", {Size=UDim2.new(0, 250, 0, 15), Position=UDim2.new(0, 65, 0, 35), BackgroundTransparency=1, Parent=LoadingCenter})
    
    local LoadingBarBG = Create("Frame", { Size = UDim2.new(0, 290, 0, 6), Position = UDim2.new(0, 15, 0, 70), BackgroundColor3 = Color3.fromRGB(40, 40, 40), Parent = LoadingCenter })
    ...
    task.spawn(function()
        ...
        local SubWrap, SubTopC, SubBotC, SubTopT, SubBotT, SubSlash = CreateSplittableText(SubtitleArea, mapName, Enum.Font.Gotham, 10, RedWhiteGradient, UDim2.new(0,0,0,0), math.min(subWidth, 250))
    ...
    function library:CreateTab(iconName)
        ...
        function tab:CreateMultiDropdown(text, options, callback)
            local DropFrame = Create("Frame", { Size = UDim2.new(1, 0, 0, 36), BackgroundColor3 = Color3.fromRGB(20, 25, 40), BackgroundTransparency = 0.3, Parent = TabPage })
            Create("UICorner", { CornerRadius = UDim.new(0, 4), Parent = DropFrame })
            local Label = Create("TextLabel", { Size = UDim2.new(1, -35, 1, 0), Position = UDim2.new(0, 5, 0, 0), BackgroundTransparency = 1, Text = text, Font = Enum.Font.Gotham, TextSize = 11, TextXAlignment = Enum.TextXAlignment.Left, TextColor3 = Color3.fromRGB(255, 255, 255), Parent = DropFrame })
            local Arrow = Create("TextLabel", { Size = UDim2.new(0, 30, 1, 0), Position = UDim2.new(1, -30, 0, 0), BackgroundTransparency = 1, Text = "▼", Font = Enum.Font.GothamBold, TextSize = 12, TextColor3 = Color3.fromRGB(200, 200, 200), Parent = DropFrame })
            local Btn = Create("TextButton", { Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Text = "", Parent = DropFrame })
            
            local selectedStates = {}
            for _, opt in ipairs(options) do selectedStates[opt] = false end
            
            local function updateLabel()
                local sels = {}
                for k, v in pairs(selectedStates) do if v then table.insert(sels, k) end end
                if #sels == 0 then Label.Text = text .. " : None"
                elseif #sels == #options then Label.Text = text .. " : All"
                else Label.Text = text .. " : " .. tostring(#sels) .. " Selected" end
            end
            updateLabel()
            
            Btn.MouseButton1Click:Connect(function()
                if library.CurrentDropdown == DropFrame then
                    library.DropFlyout.Visible = false
                ...
                local flyout = library.DropFlyout
                local fScroll = library.FlyoutScroll
                for _, v in pairs(fScroll:GetChildren()) do if v:IsA("TextButton") or v:IsA("UICorner") then v:Destroy() end end
                
                for _, opt in ipairs(options) do
                    local disp = selectedStates[opt] and ("✅ " .. opt) or opt
                    local OptBtn = Create("TextButton", { Size = UDim2.new(1, 0, 0, 24), Position = UDim2.new(0, 0, 0, 0), BackgroundColor3 = Color3.fromRGB(40, 40, 50), Text = disp, Font = Enum.Font.Gotham, TextSize = 10, TextColor3 = selectedStates[opt] and Color3.fromRGB(0, 255, 100) or Color3.fromRGB(255, 255, 255), Parent = fScroll })
                    Create("UICorner", { CornerRadius = UDim.new(0, 4), Parent = OptBtn })
                    OptBtn.MouseButton1Click:Connect(function()
                        selectedStates[opt] = not selectedStates[opt]
                        OptBtn.Text = selectedStates[opt] and ("✅ " .. opt) or opt
                        OptBtn.TextColor3 = selectedStates[opt] and Color3.fromRGB(0, 255, 100) or Color3.fromRGB(255, 255, 255)
                        updateLabel()
                        if callback then task.spawn(pcall, callback, selectedStates) end
                    end)
                end
                
                local optHeight = 24
                local numOpts = #options
                local visibleOpts = math.clamp(numOpts, 1, 7)
                local contentHeight = (numOpts * optHeight) + ((numOpts - 1) * 2)
                fScroll.CanvasSize = UDim2.new(0, 0, 0, contentHeight)
        ...
        function tab:CreateConsole(titleText)
            local ConsoleFrame = Create("Frame", { Size = UDim2.new(1, 0, 0, 150), BackgroundColor3 = Color3.fromRGB(15, 20, 30), BackgroundTransparency = 0.3, Parent = TabPage })
            Create("UICorner", { CornerRadius = UDim.new(0, 6), Parent = ConsoleFrame })
            Create("UIStroke", { Thickness = 1, Color = Color3.fromRGB(0, 150, 255), Parent = ConsoleFrame })
            
            local TitleL = Create("TextLabel", { Size = UDim2.new(1, -10, 0, 16), Position = UDim2.new(0, 5, 0, 5), BackgroundTransparency = 1, Text = titleText, Font = Enum.Font.GothamBold, TextSize = 11, TextXAlignment = Enum.TextXAlignment.Left, TextColor3 = Color3.fromRGB(0, 150, 255), Parent = ConsoleFrame })
            
            local Scroll = Create("ScrollingFrame", { Size = UDim2.new(1, -10, 1, -25), Position = UDim2.new(0, 5, 0, 23), BackgroundTransparency = 1, ScrollBarThickness = 2, Parent = ConsoleFrame })
            
            local OutputText = Create("TextLabel", { Size = UDim2.new(1, 0, 0, 0), BackgroundTransparency = 1, Text = "Waiting for data...", Font = Enum.Font.Code, TextSize = 10, TextXAlignment = Enum.TextXAlignment.Left, TextYAlignment = Enum.TextYAlignment.Top, TextColor3 = Color3.fromRGB(200, 255, 200), RichText = true, Parent = Scroll })
            ...
            OutputText:GetPropertyChangedSignal("TextBounds"):Connect(function()
                ...
                Scroll.CanvasSize = UDim2.new(0, 0, 0, OutputText.TextBounds.Y + 10)
            end)
            
            local consoleObj = {}
            function consoleObj:SetText(str)
                OutputText.Text = str
            end
...
-- TAB AUTO FARM
-- ==========================================
local TabFarm = Window:CreateTab("zap")
TabFarm:CreateSection("Auto Farm Squishy", "Teleport & Steal")

local targetRarities = {}
local rarityOptions = {"Common", "Uncommon", "Rare", "Epic", "Legendary", "Mythic", "Secret"}
TabFarm:CreateMultiDropdown("Pilih Rarity", rarityOptions, function(states)
    targetRarities = states
end)

local flySpeed = 500
-- Slider dihapus karena menggunakan Instant TP

local autoFarmSquishy = false
...
local function GetMyPlotCFrame()
    ...
    if plots then
        for _, plot in pairs(plots:GetChildren()) do
            ...
            if ownerSign then
                local ownerUI = ownerSign:FindFirstChild("OwnerSignUI", true)
                if ownerUI then
                    local nameLabel = ownerUI:FindFirstChild("PlayerName")
                    if nameLabel and (nameLabel.Text == LocalPlayer.DisplayName or nameLabel.Text == LocalPlayer.Name) then
                        local spawnBase = plot:FindFirstChild("Floors")
                        if spawnBase then
                            -- Cari titik tengah plot (biasanya Part pertama di lantai)
                            local basePart = spawnBase:FindFirstChild("Part", true)
                            if basePart then
                                return basePart.CFrame + Vector3.new(0, 5, 0)
                            end
                        end
                        return plot:GetModelCFrame() + Vector3.new(0, 10, 0)
                    end
                end
            end
        end
...
local function GetTargetSquishy()
    local animals = Workspace:FindFirstChild("Animals")
    if not animals then return nil end
    
    for _, zone in pairs(animals:GetChildren()) do
        if zone:IsA("Folder") then
            for _, sq in pairs(zone:GetChildren()) do
                if sq:IsA("Model") then
                    local rarity = sq:GetAttribute("Rarity")
                    if rarity and targetRarities[rarity] then
                        -- Check if Steal prompt exists and is enabled
                        local prompt = sq:FindFirstChild("Steal", true)
...
local tglFarm = TabFarm:CreateToggle("Auto Farm Squishy", function(state)
    autoFarmSquishy = state
    if state then
        NexzanUI:Notify({Title="Auto Farm ON", Text="Memulai berburu Squishy...", Duration=2})
        if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
            -- Save current position as return spot
            returnCFrame = LocalPlayer.Character.HumanoidRootPart.CFrame
        end
    end
end)
TabFarm:CreateKeybind("Toggle Farm Squishy", "F", function() tglFarm:Toggle() end)

task.spawn(function()
    local isWalkingToEdge = false
    local walkTween = nil

    while task.wait(0.1) do
        if autoFarmSquishy then
            pcall(function()
                local prompt = GetTargetSquishy()
                local char = LocalPlayer.Character
                ...
                if prompt and prompt.Parent then
                    -- SQUISHY KETEMU!
                    if walkTween then 
                        pcall(function() walkTween:Cancel() end) 
                        walkTween = nil
                    end
                    isWalkingToEdge = false
                    if hrp then
                        local bv = hrp:FindFirstChild("NexzanExploreVel")
                        if bv then bv:Destroy() end
                    end

                    -- INSTANT TP KE SQUISHY
                    MoveTo(prompt.Parent.CFrame + Vector3.new(0, 4, 0))
                    task.wait(0.3)
                    
                    if prompt and prompt.Enabled then
                        local spamCount = 0
                        while prompt and prompt.Enabled and prompt.Parent and autoFarmSquishy and spamCount < 20 do
                            if hrp then hrp.CFrame = prompt.Parent.CFrame + Vector3.new(0, 4, 0) end
                    ...
                    if autoFarmSquishy then
                        -- TP BALIK KE PLOT SENDIRI SECARA AKURAT
                        MoveTo(GetMyPlotCFrame())
                        task.wait(1)
                    end
                else
                    -- SQUISHY BELUM KETEMU, JALAN MENDATAR KE AREA TERAKHIR/UJUNG (Z=3000)
                    if not isWalkingToEdge and hrp then
                        isWalkingToEdge = true
                        
                        -- Menggunakan BodyVelocity untuk menahan gravitasi selama jalan mendatar
                        local bv = hrp:FindFirstChild("NexzanExploreVel")
                        if not bv then
                            bv = Instance.new("BodyVelocity")
                            bv.Name = "NexzanExploreVel"
                            bv.Velocity = Vector3.new(0, 0, 0)
                            bv.MaxForce = Vector3.new(9e9, 9e9, 9e9)
                            bv.Parent = hrp
                        end
                        
                        -- Kecepatan sedang menuju ke ujung map
                        local tInfo = TweenInfo.new(15, Enum.EasingStyle.Linear)
                        walkTween = TweenService:Create(hrp, tInfo, {CFrame = CFrame.new(hrp.Position.X, hrp.Position.Y, 3000)})
                        walkTween:Play()
                        
                        walkTween.Completed:Connect(function()
                            -- Jika mentok di ujung (Area Terakhir), dia akan diam (standby/nunggu)
                            -- sampai Squishy ke-trigger dan loop utama membatalkannya.
                        end)
                    end
                end
            end)
        else
            -- AUTO FARM MATI -> RESET STATE
            if walkTween then 
                pcall(function() walkTween:Cancel() end) 
                walkTween = nil
                isWalkingToEdge = false
...
TabFarm:CreateSection("Auto Collect Cash", "Klaim semua penghasilan Plot")

local autoCollectCash = false
local collectSpeed = 10
TabFarm:CreateSlider("Spam Speed (Collect)", 1, 50, 10, function(val) collectSpeed = val end)

local tglCash = TabFarm:CreateToggle("Auto Collect Cash", function(state) autoCollectCash = state end)
TabFarm:CreateKeybind("Toggle Collect Cash", "C", function() tglCash:Toggle() end)
                    ...
                    for _, plot in pairs(plots:GetChildren()) do
                        ...
                        if ownerSign then
                            ...
                            if ownerUI then
                                local nameLabel = ownerUI:FindFirstChild("PlayerName")
                                if nameLabel and nameLabel.Text == LocalPlayer.DisplayName or nameLabel.Text == LocalPlayer.Name then
                                    -- Ini adalah Plot milik kita, cari semua ClaimButton
                                    for _, obj in pairs(plot:GetDescendants()) do
                                        if obj.Name == "ClaimButton" and obj:IsA("Model") then
                                            local mainPart = obj:FindFirstChild("Main") or obj:FindFirstChild("Part")
                                            if mainPart and mainPart:FindFirstChildOfClass("TouchTransmitter") then
                                                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                                                    local hrp = LocalPlayer.Character.HumanoidRootPart
...
TabFarm:CreateSection("Auto Sell All", "Jual semua Squishy (Berdasarkan tas)")
local autoSellAll = false
local sellAllSpeed = 10
TabFarm:CreateSlider("Spam Speed (Sell All)", 1, 50, 10, function(val) sellAllSpeed = val end)
local tglSellAll = TabFarm:CreateToggle("Auto Sell All", function(state) autoSellAll = state end)
TabFarm:CreateKeybind("Toggle Sell All", "Z", function() tglSellAll:Toggle() end)

task.spawn(function()
    local sellRem = nil
    pcall(function() sellRem = game:GetService("ReplicatedStorage"):WaitForChild("Packages"):WaitForChild("Networker"):WaitForChild("Holder"):WaitForChild("RE/Animal/SellAll") end)
    while task.wait() do
        if autoSellAll and sellRem then
            pcall(function() sellRem:FireServer() end)
            task.wait(1 / sellAllSpeed)
        end
    end
end)

-- =========================================
-- TAB UPGRADES & REBIRTH
-- =========================================
local TabUpgrades = Window:CreateTab("trending-up")

TabUpgrades:CreateSection("Auto Upgrade Squishy", "Level up otomatis Squishy di Plot")

local autoUpgradeSquishy = false
local upgradeSpeed = 10
local maxUpgradeSlot = 10

TabUpgrades:CreateInput("Masukkan Max Slot", "Contoh: 10", function(val) 
    local n = tonumber(val) 
    if n then 
        maxUpgradeSlot = n 
        NexzanUI:Notify({Title="Slot Upgrade", Text="Max Slot: "..n, Duration=2}) 
    end 
end)
TabUpgrades:CreateSlider("Spam Speed (Upgrade)", 1, 50, 10, function(val) upgradeSpeed = val end)

local tglUpgrade = TabUpgrades:CreateToggle("Auto Upgrade Squishy", function(state) autoUpgradeSquishy = state end)
TabUpgrades:CreateKeybind("Toggle Upgrade Squishy", "V", function() tglUpgrade:Toggle() end)

task.spawn(function()
    local upgRemote = nil
    pcall(function() upgRemote = game:GetService("ReplicatedStorage"):WaitForChild("Packages"):WaitForChild("Networker"):WaitForChild("Holder"):WaitForChild("RE/PlotService/UpgradeAnimal") end)
    
    local currentUpSlot = 1
    while task.wait() do
        if autoUpgradeSquishy and upgRemote then
...
TabUpgrades:CreateSection("Auto Buy Upgrades", "Tingkatkan stats karakter")
local autoStr = false
local strSpeed = 10
TabUpgrades:CreateSlider("Spam Speed (Strength)", 1, 50, 10, function(val) strSpeed = val end)
local tglStr = TabUpgrades:CreateToggle("Auto Buy Strength +5", function(state) autoStr = state end)

local autoSpd = false
local spdSpeed = 10
TabUpgrades:CreateSlider("Spam Speed (Speed)", 1, 50, 10, function(val) spdSpeed = val end)
local tglSpd = TabUpgrades:CreateToggle("Auto Buy Speed +5", function(state) autoSpd = state end)

local autoCarry = false
local carrySpeed = 10
TabUpgrades:CreateSlider("Spam Speed (Carry)", 1, 50, 10, function(val) carrySpeed = val end)
local tglCarry = TabUpgrades:CreateToggle("Auto Upgrade Carry", function(state) autoCarry = state end)

TabUpgrades:CreateSection("Auto Rebirth", "Otomatis melakukan Rebirth")
local autoRebirth = false
local rbSpeed = 5
TabUpgrades:CreateSlider("Spam Speed (Rebirth)", 1, 50, 5, function(val) rbSpeed = val end)
local tglRb = TabUpgrades:CreateToggle("Auto Rebirth", function(state) autoRebirth = state end)

task.spawn(function()
    local upgRem = nil
    pcall(function() upgRem = game:GetService("ReplicatedStorage"):WaitForChild("Packages"):WaitForChild("Networker"):WaitForChild("Holder"):WaitForChild("RE/UpgradeService/Buy") end)
    while task.wait() do
        if autoStr and upgRem then pcall(function() upgRem:FireServer("Strength", 5) end); task.wait(1 / strSpeed) end
    end
end)
task.spawn(function()
    local upgRem = nil
    pcall(function() upgRem = game:GetService("ReplicatedStorage"):WaitForChild("Packages"):WaitForChild("Networker"):WaitForChild("Holder"):WaitForChild("RE/UpgradeService/Buy") end)
    while task.wait() do
        if autoSpd and upgRem then pcall(function() upgRem:FireServer("Speed", 5) end); task.wait(1 / spdSpeed) end
    end
end)
task.spawn(function()
    local upgRem = nil
    pcall(function() upgRem = game:GetService("ReplicatedStorage"):WaitForChild("Packages"):WaitForChild("Networker"):WaitForChild("Holder"):WaitForChild("RE/UpgradeService/Buy") end)
    while task.wait() do
        if autoCarry and upgRem then pcall(function() upgRem:FireServer("Carry", 1) end); task.wait(1 / carrySpeed) end
    end
end)
task.spawn(function()
    local rbRem = nil
    pcall(function() rbRem = game:GetService("ReplicatedStorage"):WaitForChild("Packages"):WaitForChild("Networker"):WaitForChild("Holder"):WaitForChild("RE/RebirthService/Rebirth") end)
    while task.wait() do
        if autoRebirth and rbRem then pcall(function() rbRem:FireServer() end); task.wait(1 / rbSpeed) end
    end
end)
...
-- TAB SHOP / DRILLS
-- =========================================
local TabDrill = Window:CreateTab("shopping-bag")
TabDrill:CreateSection("Drill Shop", "Beli dan Equip Drill Baru")

local drillList = {"Wooden", "Brick", "Steel", "Gold", "Glass", "Candy", "Frost", "Electric", "Slime", "Diamond", "Donut", "Fire", "Lava", "Lego", "Obsidian", "Neon", "Hacker", "Error", "Void", "Nuke", "Rainbow", "Galaxy"}
local selectedDrill = "Wooden"
TabDrill:CreateDropdown("Pilih Drill", drillList, function(opt) selectedDrill = opt end)
TabDrill:CreateButton("Buy & Equip Drill", function()
    pcall(function()
        game:GetService("ReplicatedStorage"):WaitForChild("Packages"):WaitForChild("Networker"):WaitForChild("Holder"):WaitForChild("RE/DrillService/Equip"):FireServer(selectedDrill)
        NexzanUI:Notify({Title="Drill Shop", Text="Mencoba membeli/equip: " .. selectedDrill, Duration=2})
    end)
end)
...
TabPlayer:CreateSection("Movement", "Speed & Jump")

local originalWalkSpeed = 16
TabPlayer:CreateSlider("WalkSpeed", 16, 250, 16, function(val)
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then
        LocalPlayer.Character.Humanoid.WalkSpeed = val
    end
end)

TabPlayer:CreateSlider("JumpPower", 50, 250, 50, function(val)
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then
...
TabPlayer:CreateKeybind("Toggle Infinite Jump", "J", function() tglInfJump:Toggle() end)
...
TabPlayer:CreateKeybind("Toggle No Clip", "N", function() tglNoClip:Toggle() end)
...
TabPlayer:CreateSection("Utility", "AFK & Keybinds")

local tglKB = TabPlayer:CreateToggle("Tampilkan Keybind UI", function(state)
    Window:ToggleKeybindViewer(state)
end)
TabPlayer:CreateKeybind("Toggle Keybind UI", "H", function() tglKB:Toggle() end)
...
TabPlayer:CreateKeybind("Toggle Anti AFK", "K", function() tglAntiAFK:Toggle() end)

TabPlayer:CreateSection("UI Controls", "Minimize & Close Hub")
TabPlayer:CreateKeybind("Minimize / Open Hub", "RightControl", function()
    if Window.MasterContainer then
        local mc = Window.MasterContainer
        local ml = Window.MinimizedLogo
        if mc.Visible then
            mc.Visible = false
            ml.Visible = true
        else
...
TabPlayer:CreateKeybind("Close Hub", "RightAlt", function()
...
-- TAB SQUISHY RADAR
-- ==========================================
local TabRadar = Window:CreateTab("monitor")
TabRadar:CreateSection("Live Server Monitor", "Daftar Squishy yang ada di Server")

local radarConsole = TabRadar:CreateConsole("Active Squishies List")
local radarActive = false

TabRadar:CreateToggle("Aktifkan Live Radar", function(state) 
    radarActive = state 
    if not state then radarConsole:SetText("Radar Paused.") end
end)
                ...
                if animals then
                    ...
                    for _, zone in pairs(animals:GetChildren()) do
                        if zone:IsA("Folder") then
                            for _, sq in pairs(zone:GetChildren()) do
                                if sq:IsA("Model") then
                                    local name = sq:GetAttribute("AnimalName") or "Unknown"
                                    local rarity = sq:GetAttribute("Rarity") or "Common"
                                    local expiry = sq:GetAttribute("ExpiresAt")
                                    
                                    local color = "white"
                                    if rarity == "Common" then color = "#a3a3a3"
                                    elseif rarity == "Uncommon" then color = "#4ade80"
                                    elseif rarity == "Rare" then color = "#60a5fa"
                                    elseif rarity == "Epic" then color = "#c084fc"
                                    elseif rarity == "Legendary" then color = "#fbbf24"
                                    elseif rarity == "Mythic" then color = "#f87171"
                                    elseif rarity == "Secret" then color = "#f43f5e" end
                                    
                                    local timeLeftStr = "N/A"
                                    if expiry then
                                        local diff = math.floor(expiry - os.time())
                                        if diff > 0 then timeLeftStr = diff .. "s" else timeLeftStr = "Expired" end
                                    else
                                        local tlObj = sq:FindFirstChild("TimeLeft", true)
                                        if tlObj and tlObj:IsA("TextLabel") then
                                            timeLeftStr = tlObj.Text
                                        end
                                    end
                                    
                                    local line = string.format("<font color='%s'>[%s]</font> <b>%s</b> | %s", color, rarity, name, timeLeftStr)
                    ...
                    if total == 0 then
                        radarConsole:SetText("No Squishies currently spawned in the map.")
                    else
                        table.insert(textList, 1, "<b>Total Active Squishies: " .. total .. "</b>\n")
                        radarConsole:SetText(table.concat(textList, "\n"))
                    end
                end
            end)
        end
